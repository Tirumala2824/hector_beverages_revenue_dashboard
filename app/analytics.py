...
def identify_dimensions(df: pd.DataFrame) -> list[str]:
    return [
        c
        for c in df.columns
        if c not in TIME_COLUMNS | {"Amount", "Quantity"}
...
def aggregate_data(
    df: pd.DataFrame, group_cols: list[str], time_grain: str = "monthly"
) -> pd.DataFrame:
    time_col = get_time_grain_column(df, time_grain)
    invalid = [c for c in group_cols if c not in df.columns or c in TIME_COLUMNS]
    if invalid:
        raise ValueError(f"Invalid grouping columns: {invalid}")
    group = [time_col] + group_cols
    result = (
        df.groupby(group, dropna=False)
        .agg(Revenue=("Amount", "sum"), Quantity=("Quantity", "sum"), TxnCount=("Amount", "count"))
        .reset_index()
    )
    ...
    previous = (
        result.groupby(group_cols)["Revenue"].shift(1) if group_cols else result["Revenue"].shift(1)
    )
    quantity_previous = (
        result.groupby(group_cols)["Quantity"].shift(1)
        if group_cols
        else result["Quantity"].shift(1)
    )
    result["RevenuePrevPeriod"] = previous
    result["QuantityPrevPeriod"] = quantity_previous
    result["RevenueGrowthAbs"] = result["Revenue"] - result["RevenuePrevPeriod"]
    result["RevenueGrowthPct"] = (
        result["RevenueGrowthAbs"].div(result["RevenuePrevPeriod"].replace(0, np.nan)) * 100
    )

    def bucket(row):
        pct = row["RevenueGrowthPct"]
        change = abs(row["RevenueGrowthAbs"])
        if pd.isna(pct):
            return "New/No Data"
        if pct >= 10 and change >= 500000:
            return "Strong Growth"
        if pct >= 3 and change >= 100000:
