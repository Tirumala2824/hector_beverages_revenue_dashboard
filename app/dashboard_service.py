...
class DashboardService:
    ...
    def __init__(self, settings: Settings, data_frame: Optional[pd.DataFrame] = None):
    ...
    @staticmethod
    def _parse_years(value: Optional[str]) -> list[int]:
    ...
    @staticmethod
    def _parse_date(value: Optional[str], fallback: str) -> pd.Timestamp:
    ...
    @staticmethod
    def _selected_dimensions(params: Mapping[str, Any], dimension_cols: list[str]) -> dict[str, list[str]]:
    ...
    @staticmethod
    def _dimension_metadata(df: pd.DataFrame, dimension_cols: list[str], selected: dict[str, list[str]]) -> list[dict[str, Any]]:
        metadata=[]
        for column in dimension_cols:
    ...
    def _empty_context(self, query: DashboardQuery, error_message: Optional[str] = None) -> dict[str, Any]:
    ...
    def build_context(self, query: DashboardQuery, params: Mapping[str, Any]) -> dict[str, Any]:
        ...
        selected_dimensions=self._selected_dimensions(params, dimension_cols) if not query.clear_filters else {}
        ...
        if filtered.empty:
            return {**self._empty_context(query), "selected_years": selected_years, "start_date": start_date.date().isoformat(), "end_date": end_date.date().isoformat(), "dimensions_meta": dimensions_meta}

        group_cols=[column for column in (query.group_by1, query.group_by2) if column and column in dimension_cols]
        ...
        previous_revenue=float(latest["RevenuePrevPeriod"].sum()) if latest["RevenuePrevPeriod"].notna().any() else 0.0
        previous_quantity=float(latest["QuantityPrevPeriod"].sum()) if latest["QuantityPrevPeriod"].notna().any() else 0.0
        ...
        strong=latest[latest["PerformanceBucket"].isin(["Strong Growth", "Moderate Growth"])].sort_values("RevenueGrowthAbs", ascending=False).head(20)
        decline=latest[latest["PerformanceBucket"] == "Significant Decline"].sort_values("RevenueGrowthAbs").head(20)
        return {
            **metadata,
            ...
            "breakdown_headers": group_cols + ["Revenue (Curr)", "Revenue (Prev)", "Δ Revenue", "Revenue Growth %", "Quantity", "PerformanceBucket"],
    ...
    @staticmethod
    def _build_yoy(df: pd.DataFrame, time_grain: str) -> Optional[dict[str, Any]]:
        if time_grain not in {"monthly", "quarterly"}:
            return None
        period_column="YearMonth" if time_grain == "monthly" else "YearQuarter"
        yoy=(df.groupby([period_column, "Year"], dropna=False).agg(Revenue=("Amount", "sum")).reset_index())
        pivot=yoy.pivot(index=period_column, columns="Year", values="Revenue").fillna(0)
        return {"periods": pivot.index.astype(str).tolist(), "years": [int(year) for year in pivot.columns], "values": pivot.to_dict(orient="list")}

    @staticmethod
    def _display_row(row: pd.Series, group_cols: list[str]) -> dict[str, Any]:
        result={column: ("-" if pd.isna(row[column]) else str(row[column])) for column in group_cols}
        result.update({
